...
export type CanarySummary=Pick<PlanningCanary,"id"|"title"|"batchId"|"generatedAt"|"state"|"author"|"lifecycle"> & {executionDate?:string};
export async function listPlanningCanaries(includeArchived=false):Promise<CanarySummary[]>{
 ...
 for(const c of index as PlanningCanary[])all.set(c.id,{id:c.id,title:c.title,batchId:c.batchId,generatedAt:c.generatedAt,state:c.state,author:c.author,executionDate:(c.receipt.input as {date?:string})?.date});
