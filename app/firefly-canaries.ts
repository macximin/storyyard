...
export async function listPlanningCanaries(includeArchived=false):Promise<PlanningCanary[]>{
 ...
 return [...all.values()].filter(c=>includeArchived || !getPlanningCanaryArchive(c.id)).sort((a,b)=>b.generatedAt.localeCompare(a.generatedAt));
}
