...
export async function POST(request:Request){
 ...
 const input=await request.json().catch(()=>null);const c=typeof input?.id==="string"?await getPlanningCanary(input.id):undefined;if(!c)return Response.json({error:"등록된 기획서가 아닙니다."},{status:404});const intent=decisionIntent(c,input);if(intent.error)return Response.json({error:intent.error},{status:intent.status});
...
export async function GET(request:Request){
 ...
 const c=await getPlanningCanary(new URL(request.url).searchParams.get("id")??"");if(!c)return Response.json({error:"등록된 기획서가 아닙니다."},{status:404});
