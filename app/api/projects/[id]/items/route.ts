...
export async function POST(r: Request, c: { params: Promise<{ id: string }> }) { const user=await getChatGPTUser(); const {id:projectId}=await c.params; if(!user||!(await owns(projectId,user.email)))return Response.json({error:"Not found"},{status:404}); const x=await r.json() as {kind?:string;title?:string;body?:string;meta?:string}; const item={id:crypto.randomUUID(),projectId,kind:x.kind||"character",title:x.title?.trim()||"새 항목",body:x.body?.trim()||"",meta:x.meta||"{}",updatedAt:new Date().toISOString()}; await getDb().insert(projectItems).values(item); return Response.json({item},{status:201}); }
