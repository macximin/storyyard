...
export default async function Page({params}:{params:Promise<{id:string}>}){
 const {id}=await params;const user=await requireChatGPTUser(`/review/canary/${id}`);if(user.role!=="admin")redirect("/");const c=await getPlanningCanary(id);if(!c)notFound();const decisions=await listFireflyReviewDecisions(id);
