...
export function ReviewCollection({ user, entries, mode, archiveCount, canaries = [], query = {} }: { user: Exclude<SidebarUser, null>; entries: ReviewCollectionEntry[]; mode: "board" | "archive"; archiveCount: number; canaries?: CanaryEntry[]; query?: CollectionQuery }) {
  ...
  return <main className="library-shell ff-review-collection-shell">
    ...
    <section className="ff-review-collection">
      ...
      })}</div> : <div className="ff-collection-table-wrap"><table className="ff-collection-table"><caption className="ff-visually-hidden">{mode === "archive" ? "보관된 검토" : "검토 목록"}, {filteredTotal}건</caption><thead><tr><th scope="col">상태</th><th scope="col">작품명</th><th scope="col">구분</th><th scope="col">최근 기록</th></tr></thead><tbody>{items.map(row => <tr key={row.id} id={row.id}><td><span className={`ff-status ff-status-${row.stage}`} data-status={row.status}>{row.statusLabel}</span></td><td className="ff-collection-title"><div><a className="ff-collection-title-link" href={row.href} title={row.title}>{row.title}</a>{row.links.length > 1 && <details className="ff-collection-candidates"><summary aria-label={`${row.title} 후보 ${row.links.length}개 보기`}>후보 {row.links.length}</summary><div>{row.links.map(link => <a key={link.href} href={link.href}>{link.label} · {link.title}</a>)}</div></details>}</div></td><td className="ff-collection-kind">{row.kind}</td><td className="ff-collection-date"><time dateTime={row.updatedAt}>{new Intl.DateTimeFormat("ko-KR", { timeZone: "Asia/Seoul", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(row.updatedAt))}</time></td></tr>)}</tbody></table></div>}
