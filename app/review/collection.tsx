...
export function ReviewCollection({ user, entries, mode, archiveCount, canaries = [], controls }: { user: Exclude<SidebarUser, null>; entries: ReviewCollectionEntry[]; mode: "board" | "archive"; archiveCount: number; controls?: React.ReactNode; canaries?: Array<{canary:CanarySummary;decisions:Decision[]}> }) {
  ...
  return <main className="library-shell ff-review-collection-shell">
    ...
    <section className="ff-review-collection">
      ...
      {controls}{mode === "board" ? <div className="ff-review-kanban">{columns.map((column) => {
