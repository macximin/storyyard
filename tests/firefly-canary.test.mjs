...
async function routeHarness(role='admin'){
 ...
 const dependencies={'@/app/chatgpt-auth':{runtimeSecret:name=>name==='STORYYARD_REVIEW_SYNC_TOKEN'?'read-token':'apply-token',getChatGPTUser:async()=>role?{role,id:'test-user',email:'test@example.invalid'}:null},'@/app/firefly-review-service-auth':{hasDistinctBearerAuthority},'@/app/firefly-canaries':{getPlanningCanary:id=>id===c.id?c:undefined},'@/app/firefly-canary-contract.mjs':contract,'@/db':{getDb:()=>db},'@/db/schema':{fireflyReviewDecisions:{},fireflyReviewSnapshots:{}},'drizzle-orm':{and:()=>null,desc:()=>null,eq:()=>null}};
